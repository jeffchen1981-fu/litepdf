...
TEST_CASE("RenderEngine::cancel before worker picks up skips callback", "[core][render][cancel]") {
    ...
    auto tok = engine.submit({1, 2, 1.0f, [&](fz_pixmap*, fz_context*){ ++calls; }});
    engine.cancel(tok);
...
TEST_CASE("RenderEngine::cancel_all_below_priority cancels less-urgent queued tasks",
    ...
    engine.submit({1, 1, 1.0f, [&](fz_pixmap*, fz_context*){ ++p1_calls; }});
    engine.submit({2, 2, 1.0f, [&](fz_pixmap*, fz_context*){ ++p2_calls; }});
    engine.submit({3, 3, 1.0f, [&](fz_pixmap*, fz_context*){ ++p3_calls; }});
