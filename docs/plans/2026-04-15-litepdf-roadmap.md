...
- **Mid-page search cancellation is not honored by MuPDF 1.24.11's search path. — RESOLVED in v1.1.0 (with one follow-up).** `Document::page_hits` now checks `abort_flag` at the top of every incremental `fz_search_forwards` iteration, so an in-progress per-page scan can be cut short. `SearchSession`'s teardown (tab close / session end) sets that flag, so closing a tab cancels an in-flight scan immediately. NB: the planned `fz_match_stext_page_cb` callback-abort proved to be a no-op in 1.27.2; the working path is the incremental `fz_new_search` / `fz_feed_search` / `fz_search_forwards` loop. **Follow-up:** on a *query change* the in-flight page still completes before the generation epoch discards its result — `set_query` reuses one shared `abort_flag` and resets it to 0 for the new generation, so the old worker never sees the cancel. A per-generation abort token (verified with a `ThreadPoolDispatcher` timing test) would make query-change cancellation prompt too; deferred because the cost is bounded (≤ worker-count in-flight pages, ~1–2 ms each) and the existing unit harness (`InlineDispatcher`, synchronous) cannot exercise the race. Remaining limitation: a catastrophic-backtracking regex cannot be interrupted mid-match.
