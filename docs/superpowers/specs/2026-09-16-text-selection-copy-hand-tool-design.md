...
| R14 | A word or line snap in the right margin of a non-final line selects the NEXT line's first word (Words) or that whole line (Lines) — MuPDF resolves a line's trailing boundary to the same index as the next line's first character, so the snap walks into it. Measured on MuPDF 1.27.2 against `selection.pdf` page 3: a Words click at (200, 72), right of `gamma` and inside line 1's band, copies `delta`. Chars mode is unaffected. |
