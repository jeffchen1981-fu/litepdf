...
| R13 | A word or line snap that resolves to a NON-FINAL line's TRAILING boundary selects the following line's first word (Words) or that whole line (Lines), because MuPDF gives that boundary the same index as the next line's first character. The pointer reaches it from more than the margin: anywhere right of the midpoint of the line's last glyph, and anywhere below the line's +/-size/2 band, which includes the bottom ~0.2 em of the line's own glyph boxes. The mirror case resolves to the LEADING boundary, selecting the line's first word from the top ~0.2 em of its glyphs. Measured on MuPDF 1.27.2, `selection.pdf` page 3: Words at (168.5, 67.35) inside the final `a` of `gamma` copies `delta` while (167.5, 67.35) copies `gamma`; (100, 75.0) inside `alpha`'s box copies `delta`; (150, 60.0) inside `gamma`'s box copies `alpha`. `mupdf-gl` uses the same call, so it behaves the same. The page's last line is unaffected: its trailing boundary is the end of the text, which R8 covers (measured: a Words click in its own bottom sliver copies `epsilon`). Chars mode is unaffected. Pinned by `DocumentSelection a snap at a line boundary belongs to the neighbouring line`. |
